# ble_simple_peripheral.py - Simple BLE Peripheral Driver
import bluetooth
import random
import struct
import time
    ...
    def _ad_payload(self, limited_disc=False, br_edr=False, name=None, services=None, appearance=0):
        ...
        if services:
            for uuid in services:
                b = bytes(uuid)
                if len(b) == 2:
                    _append(const(0x03), b)
                elif len(b) == 4:
                    _append(const(0x05), b)
                elif len(b) == 16:
                    _append(const(0x07), b)

        if appearance:
            _append(const(0x19), struct.pack("<h", appearance))
